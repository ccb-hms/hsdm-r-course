# Accessing NHANES with Docker {#sec-install-docker}
...
```
docker pull hmsccb/nhanes-workbench:0.0.4
```
...
`docker pull --platform linux/arm64 hmsccb/nhanes-workbench:0.0.4`

or

`docker pull --platform linux/x86_64/v8 hmsccb/nhanes-workbench:0.0.4`
...
docker \
    run \
        ...
        hmsccb/nhanes-workbench:hmsccb/nhanes-workbench:0.0.4
```
...
```
docker run  --rm --name nhanes-workbench -d  -v LOCAL_DIRECTORY:/HostData -p 8787:8787 -p 2200:22 -p 1433:1433  -e 'CONTAINER_USER_USERNAME=USER'  -e 'CONTAINER_USER_PASSWORD=PASSWORD' -e 'ACCEPT_EULA=Y' -e 'SA_PASSWORD=yourStrong(!)Password' hmsccb/nhanes-workbench:0.0.4
```
