...
Throughout these sessions we will be replicating the analysis from [Beheshti et. al. 2021](https://pubmed.ncbi.nlm.nih.gov/33892837/)[^session1-1]

[^session1-1]: Beheshti, Mahdieh et al. "Association of Diabetes and Dental Caries Among U.S. Adolescents in the NHANES Dataset." Pediatric dentistry vol. 43,2 (2021): 123-128.
...
You can find the dataset [here]() in the session 1 materials folder. Download it and place it into an appropriate location in your project folder.

We will use the `read_csv` function for reading in this `.csv` file. In a later lesson we will learn how to extract and transform this data directly from the raw NHANES database in R.
...
nhanes_ado <- read_csv(file.choose()) # <2> 
```
...
2.  It would be good to change `file.choose` to relative location of the data in your project folder.
...
This is a basic markdown table. We will see more advanced ways to make tables later. Here we are using *inline* code to fill put variables into the markdown text.
...
-   There are `r decay_or_restore` samples who either have dental decay present or dental restoration present (or both).
-   There are `r white_and_medpg` samples who are Non-Hispanic White and whose plasma glucose is greater than the median.\
:::

# Transforming data

If we take a look at Table 1a in [Beheshti et. al.](https://pubmed.ncbi.nlm.nih.gov/33892837/) we can see that all variable have been reported as categories. This is a common practice which allows the use of categorical tests such as the $\chi^2$ test. However, this means we need to transform our numeric variables into these categories.

Recall that `mutate` can be used to create new columns out of old columns. We can combine this with `cut` to create new categories.

```{r}
nhanes_ado <- nhanes_ado %>%
  mutate(
    age.cat = cut(
    age.years,
    breaks = c(13, 15, 18),
...
Try to create the categories used in the paper for plasma glucose and HbA1C.
...
Try to do the same for Family PIR. *Hint: Check the type of `family.PIR`.*

```{r}
# Solution where we deal with the text values first
# Since we are converting to categories around 1, we can just set everything >5 to 5. 
nhanes_ado <- nhanes_ado %>%
  mutate(family.PIR.lt1 = str_replace(family.PIR,"PIR value greater than or equa|Value greater than or equal to", "5.0")) %>%
  mutate(family.PIR.lt1 = as.numeric(family.PIR.lt1) < 1.0)
```

This alternate solution where we convert first seems like it could also work, but we would be replacing NA's which exist due to missing data.
```{r}
#| eval: false
# NOT A VALID SOLUTION
nhanes_ado <- nhanes_ado%>%
  mutate(family.PIR.lt1 = as.numeric(family.PIR) < 1.0) %>%
  replace_na(family.PIR.lt1 = FALSE)
```
:::
